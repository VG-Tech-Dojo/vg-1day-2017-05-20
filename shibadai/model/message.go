...
func MessageByID(db *sql.DB, id string) (*Message, error) {
	...
	// 1-1. ユーザー名を表示しよう
	if err := db.QueryRow(`select id, body, sender_name from message where id = ?`, id).Scan(&m.ID, &m.Body, &m.Name); err != nil {
		return nil, err
	}
...
func (m *Message) Insert(db *sql.DB) (*Message, error) {
	// 1-2. ユーザー名を追加しよう
	res, err := db.Exec(`insert into message (body, sender_name) values (?,?)`, m.Body, m.Name)
	if err != nil {
		return nil, err
	}
