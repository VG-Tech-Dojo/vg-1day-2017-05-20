...
func MessagesAll(db *sql.DB) ([]*Message, error) {
	...
	// 1-1. ユーザー名を表示しよう
	rows, err := db.Query(`select id, body, sender_name as sender from message`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		m := &Message{}
		// 1-1. ユーザー名を表示しよう
		if err := rows.Scan(&m.ID, &m.Body, &m.Sender); err != nil {
			return nil, err
		}
		ms = append(ms, m)
	}
...
func (m *Message) Insert(db *sql.DB) (*Message, error) {
	// 1-2. ユーザー名を追加しよう
	res, err := db.Exec(`insert into message (body, sender_name) values (?, ?)`, m.Body, m.Sender)
	if err != nil {
		return nil, err
	}
